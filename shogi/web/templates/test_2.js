...
const USI_HAND_PIECES = { "歩":"P", "銀":"S", "金":"G", "角":"B", "飛":"R" };
...
const PieceTypeToHandPieceTable = [null, HPawn, HSilver, HBishop, HRook, HGold, null, null, null, HPawn, HSilver, HBishop, HRook, HGold];
...
class Board {
	constructor() {
		...
		this.pieces_in_hand = [new Array(5), new Array(5)];
		this.move_number = 0;
		this.lastmove = null;
		this.reset();
	}
	...
	to_svg(scale, mirror=false) {
		...
		if (this.lastmove != null) {
			...
			const j = to_sq % 5;
			svg += `<rect x="${20.5 + (5 - i) * 20}" y="${10.5 + j * 20}" width="20" height="20" fill="#f6b94d" />`;
			if (from_sq < 25) {
