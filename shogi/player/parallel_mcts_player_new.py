...
class ParallelMCTSPlayer(BasePlayer):
    ...
    def select_max_ucb_child(self, board, current_node,depth):
        ...
        current_nnrate =  current_node.nnrate.to('cpu').detach().numpy().copy()
        ...
        if depth == 0 :
            eta = np.random.dirichlet([ALPHA] * len(current_nnrate))
            p = (1 - EPSILON) *  current_nnrate + EPSILON * eta
        else:
            p =  current_nnrate
    ...
    def parallel_uct_search(self):
        ...
        while True:
            ...
            # 1回プレイアウトする
            self.uct_search(board, self.current_root, 0)
            # 探索を打ち切るか確認
            with self.lock_po_info:
    ...
    def eval_node(self):
        ...
        while True:
            ...
            x1 = torch.from_numpy(np.array(eval_features1, dtype=np.float32)).to(self.device)
            x2 = torch.from_numpy(np.array(eval_features2, dtype=np.float32)).to(self.device)
            with torch.no_grad():
...
if __name__=='__main__':
    ...
    test = ParallelMCTSPlayer(device='cpu')
    test.isready()
    test.go()
